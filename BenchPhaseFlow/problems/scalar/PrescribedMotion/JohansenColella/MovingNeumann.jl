...
"""
Johansen–Colella moving-ellipse case with Dirichlet data on the exterior box and
Neumann data on the moving ellipses. Fluxes are computed from the analytic
solution projected onto the instantaneous normal of the ellipse.
"""

const BENCH_ROOT = normpath(joinpath(@__DIR__, "..", ".."))
include(joinpath(BENCH_ROOT, "utils", "convergence.jl"))

function run_moving_neumann_convergence(
    nx_list::Vector{Int},
    ny_list::Vector{Int},
    Tend::Float64;
    lx::Float64 = 3.0,
    ...
    for (nx, ny) in zip(nx_list, ny_list)
        ...
        bc_outer = Dirichlet((x,y,t)->φ_exact(x,y,t))
        bc_b = BorderConditions(Dict(:left=>bc_outer, :right=>bc_outer, :top=>bc_outer, :bottom=>bc_outer))
        interface_bc = Neumann((x,y,t)->neumann_flux(x,y,t,true))
...
function main(; csv_path=nothing, nx_list=nothing, ny_list=nothing)
    ...
    data = run_moving_neumann_convergence(nx_vals, ny_vals, Tend)
    csv_info = write_convergence_csv("JohansenColella_Moving_Neumann", data; csv_path=csv_path)
    return (data=data, csv_path=csv_info.csv_path, table=csv_info.table)
end

results = main()

@testset "Johansen-Colella moving Neumann" begin
