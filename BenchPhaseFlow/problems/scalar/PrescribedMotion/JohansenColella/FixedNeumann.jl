...
"""
Johansen–Colella fixed-domain case with Dirichlet data on the outer box and
Neumann data on the elliptical cavities. The Neumann flux is evaluated from the
analytic gradient projected onto the ellipse normal.
"""

const BENCH_ROOT = normpath(joinpath(@__DIR__, "..","..","..", ".."))
include(joinpath(BENCH_ROOT, "utils", "convergence.jl"))

function run_fixed_neumann_convergence(
    nx_list::Vector{Int},
    ny_list::Vector{Int},
    Tend::Float64;
    lx::Float64 = 3.0,
...
function main(; csv_path=nothing, nx_list=nothing, ny_list=nothing)
    ...
    data = run_fixed_neumann_convergence(nx_vals, ny_vals, Tend)
    csv_info = write_convergence_csv("JohansenColella_Fixed_Neumann", data; csv_path=csv_path)
    return (data=data, csv_path=csv_info.csv_path, table=csv_info.table)
end

results = main()

@testset "Johansen-Colella fixed Neumann" begin
